...
def parse_args(argv=None):
    ...
    parser.add_argument("--generate_levels", default="01.*01", type=str,
    ...
    parser.set_defaults(no_bar=False, display=False, resume=False, output_coco_json=False, output_web_json=False, shuffle=False,
                        benchmark=False, no_sort=True, no_hash=False, mask_proto_debug=False, crop=True, detect=False)
...
def display(img_numpy, img_gpu, masks, classes, scores, boxes, undo_transform=True,class_color=False, mask_alpha=0.45):
    ...
    try:
        if dataset.name == "YouTube VIS":
            ...
            for it, video_idx in enumerate(dataset_indices):
                ...
                for pass_idx in range(num_passes):
                    ...
                    for frame_seq_idx, (extra, (img, gt, gt_masks, h, w, num_crowd)) in enumerate(zip(extra_data, video_frames)):
                        ...
                        if args.display:
                            img_numpy, img_gpu, masks, classes, scores, boxes = prep_display(preds, img, h, w)
                            img_numpy = display(img_numpy, img_gpu, masks, classes, scores, boxes)
                        elif args.benchmark:
                            new_h, new_w = h, w
                            if new_w > 640:
            ...
            for it, image_idx in enumerate(dataset_indices):
                ...
                if args.display:
                    img_numpy, img_gpu, masks, classes, scores, boxes = prep_display(preds, img, h, w)
                    #change to GT for inspection
                    #masks = gt_masks
                    #classes = gt[:,-1]
                    #boxes = gt[:,:4]*256
                    #scores = np.ones(classes.shape[0])
                    img_numpy = display(img_numpy, img_gpu, masks, classes, scores, boxes)
                elif args.benchmark:
                ...
                if args.display:
                    if it > 1:
                        print('Avg FPS: %.4f' % (1 / frame_times.get_avg()))
                    plt.imshow(img_numpy)
                    #plt.title(str(dataset.ids[image_idx])) We have no name
                    plt.show()
                elif not args.no_bar:
