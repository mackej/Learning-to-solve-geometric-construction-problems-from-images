...
import numpy as np
import cv2
import YolactExperiment.layers.box_utils as box_utils
...
def postprocess(det_output, w, h, batch_idx=0, interpolation_mode='bilinear',
                visualize_lincomb=False, crop_masks=True, score_threshold=0, binarize_masks=True):
    """
    ...
    if cfg.preserve_aspect_ratio:
        ...
        boxes = dets['box']
        boxes = box_utils.center_size(boxes)
    ...
    boxes[:, 0], boxes[:, 2] = sanitize_coordinates(boxes[:, 0], boxes[:, 2], b_w, cast=True)
    boxes[:, 1], boxes[:, 3] = sanitize_coordinates(boxes[:, 1], boxes[:, 3], b_h, cast=True)
    boxes = boxes.long()
