...
import euclidea_datagen as euclidea_dataset
...
parser.add_argument("--epoch_size", default=1_000_00, type=int,
                        help="size of epoch, e.g. how many images will be consideren in one epoch // batch size")
...
def train(rank, args):
    ...
    dataset.euclidea.dump_class_info(args.log_folder)
    #cfg.num_classes = dataset.euclidea.get_number_od_classes()
    ...
    epoch_size = args.epoch_size // args.batch_size // args.num_gpus
    ...
    data_loader = data.DataLoader(dataset,
                                  ...
                                  #MackeJ change used to be multiprocessing_context="fork"
                                  multiprocessing_context="spawn" if args.num_workers > 1 else None,
    ...
    try:
        for epoch in range(num_epochs):
            ...
            while True:
                ...
                # Freeze and fine-tune layers during the training
                if hasattr(cfg, 'fine_tune_layers_over_time'):
                    for fine_t in cfg.fine_tune_layers_over_time:
                        if iteration == fine_t['iteration']:
                            net.fine_tune_layers(fine_tune_layers=fine_t['fine_tune'])
...
if __name__ == '__main__':
    ...
    with open(args.log_folder + os.path.sep + 'config.txt',"w") as f:
        sys.stdout = f
        cfg.print()
    with open(args.log_folder + os.path.sep + 'args.txt',"w") as f:
        sys.stdout = f
        print(args)
