...
import YolactInference_SingleModel
...
def TestModel(levels, model_path, episodes,  history_size,
    ...
    model = YolactInference_SingleModel.YolactSingleModelInference.load_model(model_path, output_to=output_to)
    ...
    for level in range(len(levels)):
        ...
        if len(unfinished_envs) != 0 and log_fails:
            ...
            unfinished_envs = []
    csv_output = open(os.path.join(os.path.join(os.path.dirname(model_path),
    ...
    sourceFile = open(os.path.join(os.path.join(os.path.dirname(model_path),
                                                          "time_elapsed_per_detection_{}.csv".format(datetime.now().strftime("%Y-%m-%d-%H-%M-%S")))), "w+")
    print("DETECTION TIME,{}".format(model.sum_elapsed / model.elapsed_cnt), file=sourceFile)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_path", default="weights/LR_test_3_alpha_2021_06_08/Euclidea_79_1000000.pth", type=str,
    ...
    parser.add_argument("--white_visualization", default=0, type=int,
